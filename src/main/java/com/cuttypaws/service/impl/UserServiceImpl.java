...
public class UserServiceImpl implements UserService {
    ...
    public UserDto getUserByIdWithAddress(UUID id) {
        return userRepo.findByIdWithAddress(id)
                .map(userMapper::mapUserToDtoPlusAddress)
                .orElse(null);
    }
    ...
    public UserResponse getAllSellers() {
        List<User> users = userRepo.findAllSellers();
        List<UserDto> userDtos = users.stream()
                .map(userMapper::mapUserToDtoBasic)
                .collect(Collectors.toList());
    ...
    @Override
    public UserResponse getSellerWithProducts(UUID sellerId) {
        User seller = userRepo.findSellerWithProductsById(sellerId)
                .orElseThrow(() -> new NotFoundException("Seller not found"));

        UserDto sellerDto = userMapper.mapUserToDtoBasic(seller);
        List<ProductDto> productDtos = seller.getProducts().stream()
                .map(productMapper::mapProductToDtoBasic)
                .collect(Collectors.toList());

        return UserResponse.builder()
                .status(200)
                .message("Seller with products retrieved successfully")
                .user(sellerDto)
    ...
    private UserRole determineUserRole(String role) {
        ...
        return switch (role.toLowerCase()) {
            ...
            case "role_seller" -> UserRole.ROLE_SELLER;
